<template>
  <div class="app-container">
    ...
    <el-form-item label="课程类别">
        <el-select
        v-model="courseQueryVo.oneSubjectId"
        placeholder="请选择"
        ...
        <!-- 二级分类 -->
        <el-select v-model="courseQueryVo.twoSubjectId" placeholder="请选择">
        <el-option
...
export default {
  ...
  methods: {
    ...
    fetchData(current = 1) { // 调用api层获取数据库中的数据
      ...
      course.getPageList(this.current, this.limit, this.courseQueryVo)
        .then(response => {
            // debugger 设置断点调试
    ...
    subjectLevelOneChanged(value) {
      for (let i = 0; i < this.oneSubjectList.length; i++) {
        if (this.oneSubjectList[i].id === value) {
          ...
          this.courseQueryVo.twoSubjectId = ''
        }
      }
    },
    //重置数据
